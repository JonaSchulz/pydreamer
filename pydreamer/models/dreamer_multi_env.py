...
class DreamerMultiEnv(nn.Module):
    ...
    def init_optimizers(self, lr, lr_actor=None, lr_critic=None, eps=1e-5):
        if not self.probe_gradients:
            ...
            optimizer_actor = [torch.optim.AdamW(self.ac[i].actor.parameters(), lr=lr_actor or lr, eps=eps)
                               for i in range(self.num_actors)]
            optimizer_critic = [torch.optim.AdamW(self.ac[i].critic.parameters(), lr=lr_critic or lr, eps=eps)
                                for i in range(self.num_actors)]
    ...
    def grad_clip(self, grad_clip, grad_clip_ac=None):
        if not self.probe_gradients:
            grad_metrics = {
                ...
                'grad_norm_actor': [nn.utils.clip_grad_norm_(self.ac[i].actor.parameters(), grad_clip_ac or grad_clip)
                                    for i in range(self.num_actors)],
                'grad_norm_critic': [nn.utils.clip_grad_norm_(self.ac[i].critic.parameters(), grad_clip_ac or grad_clip)
                                     for i in range(self.num_actors)],
            }
        else:
            grad_metrics = {
    ...
    def inference(self,
        ...
        action_distr = self.ac[env_index].forward_actor(feature)  # (1,B,A)
        value = self.ac[env_index].forward_value(feature)  # (1,B)
    ...
    def training_step(self,
        ...
        features_dream, actions_dream, rewards_dream, terminals_dream = \
            self.dream(in_state_dream, H, self.ac[env_index].actor_grad == 'dynamics')  # (H+1,TBI,D)
        (loss_actor, loss_critic), metrics_ac, tensors_ac = \
            self.ac[env_index].training_step(features_dream.detach(),
        ...
        if do_dream_tensors and self.wm.decoder.image is not None:
            with torch.no_grad():  # careful not to invoke modules first time under no_grad (https://github.com/pytorch/pytorch/issues/60164)
                ...
                _, _, tensors_ac = self.ac[env_index].training_step(features_dream, actions_dream, rewards_dream.mean, terminals_dream.mean, log_only=True)
    ...
    def dream(self, in_state: StateB, imag_horizon: int, dynamics_gradients=False, env_index: int = 0):
        features = []
        actions = []
        state = in_state
        ...
        for i in range(imag_horizon):
            ...
            action_dist = self.ac[env_index].forward_actor(feature)
    ...
    def __str__(self):
        ...
        for submodel in (self.wm.encoder, self.wm.decoder, self.wm.core, self.ac[0], self.probe_model):
