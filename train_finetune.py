...
def run(conf):
    ...
    # resume_step = tools.mlflow_load_checkpoint(model)
    resume_step = tools.load_checkpoint(model, conf.model_checkpoint)
        ...
        while True:
            with timer('total'):
                ...
                if steps == conf.n_steps_wm:
                    info("Finetuning world model complete. Only actor training from here on.")
