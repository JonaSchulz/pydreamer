...
def run(conf):
    ...
    data_iter = iter(DataLoader(WorkerInfoPreprocess(preprocess(data)),
                                ...
                                prefetch_factor=20 if conf.data_workers else None,
            ...
            with timer('total'):
                ...
                with timer('forward'):
                    with autocast(enabled=conf.amp):
                        ...
                        losses, new_state, loss_metrics, tensors = \
                            model.training_step_wm(
                                obs,
                                state,
                ...
                with timer('other'):
                    ...
                    if online_data and steps % conf.logbatch_interval == 0:
                        data_train_stats = DataSequential(MlflowEpisodeRepository(input_dirs), conf.batch_length,
                                                          conf.batch_size)
                ...
                with timer('eval'):
                    if conf.eval_interval and steps % conf.eval_interval == 0:
                        try:
                            ...
                            data_test = DataSequential(MlflowEpisodeRepository(test_dirs), conf.batch_length,
                                                       conf.test_batch_size, skip_first=False,
                                                       reset_interval=conf.reset_interval)
                            ...
                            evaluate('test', steps, model, test_iter, device, conf.test_batches, conf.iwae_samples,
                                     conf.keep_state, conf.test_save_size, conf)

                            # Eval = no state reset, multisampling
                            data_eval = DataSequential(MlflowEpisodeRepository(eval_dirs), conf.batch_length,
                                                       conf.eval_batch_size, skip_first=False)
    ...
    for i_batch in range(eval_batches):
        with torch.no_grad():
            ...
            if n_continued_episodes > 0:
                with autocast(enabled=conf.amp):
                    _, _, _, tensors_im = \
                        model.training_step_wm(obs,  # observation will be ignored in forward pass because of imagine=True
                                               state,
            ...
            with autocast(enabled=conf.amp):
                ...
                _, state, loss_metrics, tensors = \
                    model.training_step_wm(obs,
                                           state,
                                           iwae_samples=eval_samples,
                                           imag_horizon=conf.imag_horizon,
                                           do_image_pred=True)
